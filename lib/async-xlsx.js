...
module.exports = {
    ...
    }, parseFileAsyncArray: function (mixed, options, callback) {
        var ws;
        if (typeof mixed === 'string') ws = XLSX.readFile(mixed, options);
        else ws = XLSX.read(mixed, options);
        callback(_.map(ws.Sheets, function (sheet, name) {
    ...
    parseFileAsyncCSV: function (mixed, options, optionsCSV, callback) {
        var ws;
        if (typeof mixed === 'string') ws = XLSX.readFile(mixed, options);
        else ws = XLSX.read(mixed, options);
        callback(_.map(ws.Sheets, function (sheet) {
            return XLSX.utils.sheet_to_csv(sheet, optionsCSV);
        }));
    },
